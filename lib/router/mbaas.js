...
function initRouter(mediator, authResponseExclusionList, expressSessionMiddleware) {
  ...
  router.route('/').get(function(req, res) {
    mediator.request('wfm:user:list').then(function(result) {
      if (result instanceof Error) {
        mediator.publish('error:wfm:user:list', result);
        return res.status(500).json({message: "Unexpected error when getting the list of users. " + result.toString()});
      }
      res.json(result);
  ...
  router.route('/:id').get(function(req, res) {
    ...
    mediator.request('wfm:user:read', userId).then(function(result) {
      if (result instanceof Error) {
        mediator.publish('error:wfm:user:read', result);
        return res.status(500).json({message: "Unexpected error when reading a user. " + result.toString()});
      }
      res.json(result);
  ...
  router.route('/:id').put(function(req, res) {
    ...
    mediator.request('wfm:user:update', user, {uid: userId}).then(function(result) {
      if (result instanceof Error) {
        mediator.publish('error:wfm:user:update', user);
        return res.status(500).json({message: "Unexpected error when updating a user. " + result.toString()});
      }
      res.json(result);
  ...
  router.route('/').post(function(req, res) {
    ...
    mediator.request('wfm:user:create', user).then(function(result) {
      if (result instanceof Error) {
        mediator.publish('error:wfm:user:create', user);
        return res.status(500).json({message: "Unexpected error when creating a user. " + result.toString()});
      }
      res.json(result);
  ...
  router.route('/:id').delete(function(req, res) {
    ...
    mediator.request('wfm:user:delete', userId).then(function(result) {
      if (result instanceof Error) {
        mediator.publish('error:wfm:user:delete', user);
        return res.status(500).json({message: "Unexpected error when deleting a user. " + result.toString()});
      }
      res.json(result);
